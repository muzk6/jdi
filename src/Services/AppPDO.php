...
class AppPDO
{
    ...
    public function __construct($connection, $open_log = false, string $table = '')
    {
        ...
        if ($this->open_log) {
            ...
            if (!empty($binds)) {
                ...
                foreach ($binds as $k => $v) {
                    ...
                    if ($indexed) {
                        $sql_log = preg_replace('/\?/', $v, $sql_log, 1);
                    } else {
                        if ($k[0] != ':') { // add leading colon if it was left out
                            $k = ':' . $k;
                        }
                        $sql_log = str_replace($k, $v, $sql_log);
                    }
                }
            }

            Svc::log()->push('sql', $sql_log, 'sql');
        }
    ...
    {
        $db_table = explode('.', $table ?: $this->table);
     ...
     * @return string
     */
    protected function parseOrderBy(string $order_by)
    {
     ...
     * @return array|false 无记录时返回 false
     */
    public function selectOne(string $columns, $where, string $table = '', string $order_by = '')
    {
        list($placeholder, $binds) = $this->parseWhere($where);

        $table = $this->parseTable($table);
        $order_by = $this->parseOrderBy($order_by);
        $sql = "SELECT {$columns} FROM {$table}{$placeholder} {$order_by}";
     ...
     * @return array 无记录时返回空数组 []
     */
    public function selectAll(string $columns, $where, string $table = '', string $order_by = '', array $limit = [])
    {
        list($placeholder, $binds) = $this->parseWhere($where);
        ...
        $sql = "SELECT {$columns} FROM {$table}{$placeholder} {$order_by} {$limit}";
    ...
    public function insert(array $data, string $table = '', $ignore = false)
    {
        ...
        $table = $this->parseTable($table);
        $sql = "INSERT{$sql_ignore} INTO {$table}({$sql_column}) VALUES {$sql_values}";
    ...
    public function update(array $data, $where, string $table = '')
    {
        ...
        foreach ($data as $k => $v) {
            if (isset($v[0]) && is_array($v)) {
                if (isset($v[1])) { // 参数绑定
                    if (is_array($v[1])) { // ['inc', [1]]
                        $set_binds = array_merge($set_binds, $v[1]);
                    } else { // ['inc', 1]
                        $set_binds = array_merge($set_binds, array_slice($v, 1));
                    }
                }
        ...
        $table = $this->parseTable($table);
        $sql = "UPDATE {$table} SET {$sql_set}{$placeholder}";
        $result = $this->query($sql, array_merge($set_binds, $binds));
